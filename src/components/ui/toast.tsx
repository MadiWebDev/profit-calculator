...
>(({ className, variant, onOpenChange, ...props }, ref) => {
  ...
  const [closeDirection, setCloseDirection] = React.useState({ x: 0, y: 0 });
  ...
  const handleTouchEnd = () => {
    ...
    // Only dismiss on left, right, or top swipes (not bottom)
    const shouldDismiss = 
      absX > threshold || // Left or right swipe
      (absY > threshold && touchOffset.y < 0); // Top swipe only (negative Y)

    if (shouldDismiss) {
      // Calculate the direction to animate out
      const multiplier = 3; // Extend the animation distance
      setCloseDirection({
        x: touchOffset.x * multiplier,
        y: touchOffset.y * multiplier
      });
      setIsClosing(true);
      
      // Trigger close after animation starts
      setTimeout(() => {
        onOpenChange?.(false);
      }, 200);
    }
  ...
  return (
    <ToastPrimitives.Root
      ...
      style={{
        transform: isClosing
          ? `translate(${closeDirection.x}px, ${closeDirection.y}px)`
          : isDragging 
          ? `translate(${touchOffset.x}px, ${touchOffset.y}px)` 
          : undefined,
        opacity: isClosing ? 0 : undefined,
        transition: isClosing ? 'all 0.2s ease-out' : isDragging ? 'none' : undefined,
      }}
